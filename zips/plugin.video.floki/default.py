...
def INDEX(url):
	...
	for a in all_vids:
		...
		addDir(name+'  [COLOR dodgerblue]%s[/COLOR]  [B]%s[/B]'%(qual,rat),url,2,thumb,fanart,'')
	try:
		part = regex_from_to(open,"<div class='resppages'>","<span")
		np = regex_from_to(part,'<a href="','"')
		addDir('[COLOR dodgerblue]NEXT PAGE>[/COLOR]',np,1,icon,fanart,'')
	except:
		pass
...
def SEARCH():
	...
	kb.setHeading('Enter Your Search') # optional
	...
	if (kb.isConfirmed()):
		...
		for a in all_vids:
			...
			thumb= regex_from_to(a,'src="','"')
			qual = regex_from_to(a,'25px;">','<')
			addDir(name+'  [COLOR dodgerblue]%s[/COLOR]'%qual,url,2,thumb,fanart,'')
			xbmcplugin.setContent(int(sys.argv[1]), 'movies')
			xbmc.executebuiltin("Container.SetViewMode(500)")
