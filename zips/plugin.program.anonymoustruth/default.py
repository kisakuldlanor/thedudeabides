...
def maintMenu(view=None):
	...
	if includevid == 'true':
		...
		addFile('--- Include Kratos: %s' % includekra.replace('true',on).replace('false',off), 'togglecache', 'includekratos', icon=ICONMAINT, themeit=THEME3)
...
def toggleCache(state):
	cachelist = ['includekratos', 'includevideo', 'includeall', 'includebob', 'includephoenix', 'includespecto', 'includegenesis', 'includeexodus', 'includeonechan', 'includesalts', 'includesaltslite']
