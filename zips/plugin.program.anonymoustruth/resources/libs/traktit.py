...
ORDER          = ['kratos', 'exodus', 'metalliq', 'salts', 'saltshd', 'velocity', 'velocitykids', 'meta', 'royalwe', 'specto', 'trakt']
...
TRAKTID = { 
	'kratos': {
		...
		'file'     : os.path.join(TRAKTFOLD, 'kratosfucksducky_trakt'),
		'settings' : os.path.join(ADDOND, 'plugin.video.kratos', 'settings.xml'),
		'default'  : 'trakt.user',
		'data'     : ['trakt.user', 'trakt.refresh', 'trakt.token'],
		'activate' : 'RunPlugin(plugin://plugin.video.kratosfucksducky/?action=authTrakt)'},
	'exodus': {
		'name'     : 'Exodus',
