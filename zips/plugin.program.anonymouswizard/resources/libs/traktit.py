...
ORDER          = ['wargames', 'exodus', 'metalliq', 'salts', 'saltshd', 'velocity', 'velocitykids', 'meta', 'royalwe', 'specto', 'trakt']

TRAKTID = { 
	'wargames': {
		'name'     : 'WarGames',
		'plugin'   : 'plugin.video.wargames',
		'saved'    : 'exodus',
		'path'     : os.path.join(ADDONS, 'plugin.video.wargames'),
		'icon'     : os.path.join(ADDONS, 'plugin.video.wargames', 'icon.png'),
		'fanart'   : os.path.join(ADDONS, 'plugin.video.wargames', 'fanart.jpg'),
		'file'     : os.path.join(TRAKTFOLD, 'wargames_trakt'),
		'settings' : os.path.join(ADDOND, 'plugin.video.wargames', 'settings.xml'),
		'default'  : 'trakt.user',
		'data'     : ['trakt.user', 'trakt.refresh', 'trakt.token'],
		'activate' : 'RunPlugin(plugin://plugin.video.wargames/?action=authTrakt)'},
	'exodus': {
		'name'     : 'Exodus',
